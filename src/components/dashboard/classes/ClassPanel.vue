...
// ClassPanel — manage Classes & Lessons inside a Course
// Admin (canManageClasses=true): add/delete Classes + Lessons & Resources
// Tutor (canManageClasses=false): add/delete Lessons & Resources only
...
const { data, isLoading } = useQuery({
  queryKey: ['classes', props.courseId],
  queryFn: () => ClassService.findByCourse(props.courseId),
  staleTime: 1000 * 60 * 2,
})
...
const { handleSubmit: handleClassSubmit, isSubmitting: classSubmitting, resetForm: resetClassForm } = useForm({
  validationSchema: toTypedSchema(CreateClassSchema),
})
...
const { mutate: createClass, isPending: creatingClass } = useMutation({
  mutationFn: (data: any) => ClassService.create(props.courseId, data),
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ['classes', props.courseId] })
...
// ── Lessons ───────────────────────────────────────────────
const showAddLesson = ref<number | null>(null)
...
const { mutate: createLesson, isPending: creatingLesson } = useMutation({
  mutationFn: ({ classId, data }: { classId: number; data: any }) =>
    LessonService.create(classId, data),
  onSuccess: (_, vars) => {
    queryClient.invalidateQueries({ queryKey: ['lessons', vars.classId] })
  ...
  onError: (err: any) => toast.error(err?.response?.data?.message ?? 'Gagal menambahkan pertemuan'),
})

const submitLesson = (classId: number) => {
  if (!lessonTitle.value.trim()) return toast.error('Judul pertemuan wajib diisi')
  
  // Convert datetime-local to ISO8601 with timezone
  let startTimeISO = null
  if (lessonStart.value) {
    const dt = new Date(lessonStart.value)
    startTimeISO = dt.toISOString()
  }
  
  createLesson({
    classId,
    data: { title: lessonTitle.value, lessonOrder: lessonOrder.value, startTime: startTimeISO },
  })
}

const { mutate: deleteLesson } = useMutation({
  mutationFn: ({ id, classId: _cid }: { id: number; classId: number }) => LessonService.remove(id),
  onSuccess: (_, vars) => {
    queryClient.invalidateQueries({ queryKey: ['lessons', vars.classId] })
    toast.success('Pertemuan dihapus')
  },
  onError: (err: any) => toast.error(err?.response?.data?.message ?? 'Gagal menghapus pertemuan'),
})
</script>
...
<template>
  <div class="space-y-3">
    ...
    <!-- Add class form (admin only) -->
    <div v-if="canManageClasses && showAddClass" class="bg-primary/5 border border-primary/20 rounded-xl p-4 space-y-3">
      <p class="text-sm font-semibold text-primary">Kelas Baru</p>
      <div>
        <input
          v-model="className"
          type="text"
          placeholder="Nama kelas (misal: Matematika, Bahasa Indonesia)..."
    ...
    <div v-else-if="classList.length === 0" class="py-10 text-center text-gray-400 border border-dashed rounded-xl">
      ...
      <p v-if="!canManageClasses" class="text-xs mt-1 text-gray-400">Hubungi admin untuk menambahkan kelas.</p>
    </div>

    <!-- Class list -->
    <div v-else class="space-y-2">
      <div v-for="cls in classList" :key="cls.id" class="border rounded-xl bg-white overflow-hidden">
        <!-- Class row -->
        <div class="flex items-center gap-3 p-4">
          <button @click="toggleExpand(cls.id)" class="flex items-center gap-3 flex-1 min-w-0 text-left">
            ...
            <div class="min-w-0">
              ...
              <p class="text-xs text-gray-400">{{ cls.lessonCount ?? 0 }} pertemuan</p>
            </div>
            <Icon
        ...
        <div v-if="expandedId === cls.id" class="border-t px-4 pb-4 pt-3 space-y-3 bg-gray-50/50">
          ...
          >
            ...
            {{ showAddLesson === cls.id ? 'Batal' : 'Tambah Pertemuan' }}
          </button>
          ...
          <div v-if="showAddLesson === cls.id" class="bg-white border rounded-lg p-3 space-y-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                ...
                <input v-model.number="lessonOrder" type="number" min="1"
                  class="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label class="text-xs text-gray-500 mb-1 block">Waktu Mulai (opsional)</label>
                <input v-model="lessonStart" type="datetime-local"
                  class="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
            </div>
            <div class="flex justify-end gap-2">
              ...
              >
                {{ creatingLesson ? 'Menyimpan...' : 'Simpan Pertemuan' }}
              </button>
            </div>
          </div>
        </div>
