...
import { CourseResourceService, type ResourceType } from '../../../../services/course.resource'
...
const { data, isLoading } = useQuery({
  queryKey: ['course-lessons', props.classId],
  queryFn: () => CourseLessonService.findByClass(props.classId),
  staleTime: 1000 * 60 * 2,
})
...
// Expanded lesson (to show resources)
...
const { mutate: createResource, isPending: creatingResource } = useMutation({
  mutationFn: ({ lessonId, data }: { lessonId: number; data: any }) =>
    CourseResourceService.create(lessonId, data),
  onSuccess: (_, vars) => {
    queryClient.invalidateQueries({ queryKey: ['lesson-resources', vars.lessonId] })
...
const { mutate: deleteResource } = useMutation({
  mutationFn: ({ id, lessonId: _lid }: { id: number; lessonId: number }) =>
    CourseResourceService.remove(id),
  onSuccess: (_, vars) => {
    queryClient.invalidateQueries({ queryKey: ['lesson-resources', vars.lessonId] })
    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        <div v-if="expandedLesson === lesson.id" class="border-t bg-gray-50/60 px-3 pb-3 pt-2 space-y-2">
          <ResourceList :lesson-id="lesson.id" @delete="(id) => deleteResource({ id, lessonId: lesson.id })" />

          <!-- Add resource toggle -->
          <button
            @click="showAddResource = showAddResource === lesson.id ? null : lesson.id; resTitle = ''; resUrl = ''; resType = 'video'"
